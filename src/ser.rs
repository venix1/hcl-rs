//! Serialize a `Body` structure into HCL data.

use crate::{Attribute, Block, BlockLabel, Body, Result, Structure, Value};
use std::io;

/// A serializer for HCL `Body` structures.
...
/// Serialize the given `Body` as an HCL byte vector.
...
/// Serialize the given `Body` as an HCL string.
...
/// Serialize the given `Body` as HCL into the IO stream.
///
/// # Errors
///
